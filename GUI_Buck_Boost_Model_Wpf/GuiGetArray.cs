...
namespace GUI_Buck_Boost_Model_Wpf
{
    public class GuiGetArray : Object
    {
        ...
        #region Constructors
        public GuiGetArray(System.Windows.Controls.TextBox[] textbox, byte itemNumber)
        {
            _txt = textbox;
        ...
        public GuiGetArray(System.Windows.Controls.TextBox[] textbox, byte itemNumber, int q_Value)
        {
            _txt = textbox;
            _dataArraySize = _txt.Length;
            _cmdNum = 0x05;
        ...
        public GuiGetArray(System.Windows.Controls.TextBox[] textbox, byte itemNumber, int R1, int R2)
        {
            _txt = textbox;
            _dataArraySize = _txt.Length;
            _dataArray = new double[_dataArraySize];
        ...
        public void ReadBuffer(byte[] buffer)
        {
            ...
            for (int i = 0; i < _dataArraySize; i++)
            {
                if (_guiValidType == 1)
                {
                    ...
                    //xxx _txt[i].Dispatcher.Invoke(new EventHandler(delegate{_txt[i].Text = toWrite;}));
                    this.txt[i].Dispatcher.Invoke((Action)(() => { _dataArray[i].ToString(); }));
                }
                else if (_guiValidType == 2)
                {
                    ...
                    //xxx _txt[i].Dispatcher.Invoke(new EventHandler(delegate{_txt[i].Text = toWrite;}));
                    this.txt[i].Dispatcher.Invoke((Action)(() => { _txt[i].Text = toWrite; }));
                }
                else if (_guiValidType == 3)
                {
                    _dataArray[i] = (byteBuffer[2 * i] + byteBuffer[2 * i + 1] * 256) * _Kv / Math.Pow(2, _qValue);// *8;
                    string toWrite = _dataArray[i].ToString("#.0000");
                    //xxx_txt[i].Dispatcher.Invoke(new EventHandler(delegate{_txt[i].Text = toWrite;}));
                    this.txt[i].Dispatcher.Invoke((Action)(() => { _txt[i].Text = toWrite; }));
                }
            }
